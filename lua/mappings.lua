...
map("n", "gD", vim.lsp.buf.declaration, { desc = "Go to declaration", noremap = true })
...
map("n", "<leader>dy", function()
    dap_ui.toggle()
end, { desc = "Toggle DAP UI", nowait = true }
)
map("n", "<F9>", function()
    dap.continue()
end, { desc = "Debugger continue", nowait = true }
)
map("n", "<F10>", function()
    dap.step_over()
...
map("n", "<leader>dd", function()
    dap.repl.clear()
end, { desc = "Clear REPL", nowait = true }
)
...
map("n", "<leader>dl", function()
    dap.run_last()
end, { desc = "Debug last run", nowait = true }
)
...
map("n", "<leader>st", "<CMD>Telescope themes<CR>", { desc = "Switch colorscheme" })
map("n", "<esc>", vim.cmd.noh)
map("n", "<leader>u", "<CMD>UndotreeToggle<CR>", { desc = "Undo tree" })
map("n", "<leader>rs", "<CMD>colorscheme pywal16<CR>", { desc = "Pywal" })
map("n", "<leader>z", require('simple-zoom').toggle_zoom, { desc = "Zoom" })
